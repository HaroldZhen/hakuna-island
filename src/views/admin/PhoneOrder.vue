...
<script>
import { hexAxios, api, frontAPI } from '@/response/hexAxios';
...
export default {
  ...
  methods: {
    ...
    getCart() {
      hexAxios.get(frontAPI.cart.list()).then((res) => {
    ...
    addToCart(product) {
      ...
      hexAxios.post(frontAPI.cart.add(), data).then((res) => {
    ...
    delCart(pid) {
      hexAxios.delete(frontAPI.cart.delete(pid)).then((res) => {
        const { success, message } = res.data;
        if (success) {
          this.$swal({
            title: message,
    ...
    onSubmit() {
      ...
      hexAxios.post(frontAPI.order.create(), data).then((res) => {
